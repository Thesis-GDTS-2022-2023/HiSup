...
class BuildingDetector(nn.Module):
    ...
    def forward_test(self, images, annotations = None):
        ...
        # print(mask_pred.shape,jloc_pred.shape)
        jloc_convex_pred1 = jloc_pred[:, 2:3]
        jloc_concave_pred1 = jloc_pred[:, 1:2]
        ...
        # print("jloc_pred", jloc_pred.shape, jloc_pred[0])
        # print("-----------------------")
        # print("joff_pred", joff_pred.shape, joff_pred[0])
        # print("-----------------------")
        # print("jloc_convex_pred", jloc_convex_pred1.shape, jloc_convex_pred1[0])
        # print("-----------------------")
        # print("jloc_concave_pred", jloc_concave_pred1.shape, jloc_concave_pred1[0])
        # print("-----------------------")
        # print("mask_pred", mask_pred.shape, mask_pred)
        # print("-----------------------")
        # print("remask_pred", remask_pred.shape,remask_pred)
    ...
    def forward_train(self, images, annotations = None):
        ...
        outputs, features = self.backbone(images) # extract Fb
        ...
        mask_feature = self.mask_head(features) # extract Fseg
        jloc_feature = self.jloc_head(features) # extract Fver
        afm_feature = self.afm_head(features) # extract Fafm

        # print("afm_feat:",afm_feature.shape)
        # print("--------------------------------")
        # print(jloc_feature.shape)
        ...
        remask_pred = self.final_conv(torch.cat((features, afm_conv), dim=1)) # concatenate F*afm to Fb
